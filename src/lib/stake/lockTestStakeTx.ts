...
import type { LockTxStatus } from "@/lib/stake/stakeSessionStorage";
...
/**
 * Send a small native test ETH payment on Base Sepolia to lock a test stake.
 * Throws if preflight fails, user rejects, or receipt fails.
 */
export async function sendLockTestStakeTx(
  ...
  return { hash, lockTxStatus: "confirmed", treasuryAddress: treasury };
}
